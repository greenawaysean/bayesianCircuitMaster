...
savepath = path.join(getcwd(), 'data', 'BO', 'comparing_FOM_F', 'real_machine', 'toronto', f"{NB_CIRCS_FOM*NB_SHOTS_FOM}_experiments")
...
_vals = [0.1*np.pi*np.random.rand() for i in range(18)]
...
while not done:
    try:
        exp_opt_true_est_circ = TrueFidelityEst(rev_chi_prob_dist, ansatz, nqubits=3,
...
FOM_diff = []
for i in range(50):
    # x = np.real(true_F - fom_est_circ.calculate_fidelity(_vals, length=NB_CIRCS_FOM))
...
# a1,b1,_ = plt.hist(F_diff, weights=weights, bins=5, alpha=0.85)
...
# a2,b2,_ = plt.hist(FOM_diff, weights=weights, bins=5, alpha=0.85)
