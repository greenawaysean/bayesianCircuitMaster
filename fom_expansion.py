...
def generate_Bmat(nqubits, order):
    ...
    for k in range(order+1):
        ...
        if B_inv is None:
            ...
            B_inv += _alpha*_beta*X_k
    # if order < nqubits:
    #     for j in range(order+1, nqubits+1):
    #         _beta = (-1/6)**j
    #         _op = tensor([qeye(4)]*nqubits)
    #         B_inv += _alpha*_beta*choose(nqubits, j)*_op
    return B_inv.full()
...
if __name__ == "__main__":
    ...
    for order in range(nqubits+1):
        ...
        init_fom = figure_of_merit(U, U, nqubits, Bmat, basis=rho_basis)
        ...
        filename = path.join(getcwd(), 'data', 'FOM_expansions_2')
